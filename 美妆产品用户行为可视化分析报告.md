...
用户质量分析旨在通过用户的消费能力、活跃度、忠诚度等维度对用户进行分层，识别高价值用户群体，为精细化运营提供依据。例如，基于前述的用户购买频次统计就可以判断用户的消费能力。接下来我们使用ARPU/ARPPU模型来进行用户质量分析，帮助量化用户商业价值。

#### (1)用户ARPU指标计算

ARPU(Average Revenue Per User) ：平均每用户收入。表示每日的收益在所有活跃用户中的转化。详细的描述为，每日的所有收益与每日的活跃的用户数量有关，因此可以通过ARPU来计算，每日活跃用户对每日收益的转化效率。

该数据集中没有对金额的体现。因此，基于数据可得性和数据相关性，我们可以对ARPU这个指标做一些简化，改为度量平台活跃用户每日平均消费次数。
...
```sql
-- 创建每日ARPU指标表
CREATE TABLE daily_arpu (
    date CHAR(10),
    total_buy_count INT,  -- 当日总购买次数
    active_user_count INT,  -- 当日活跃用户数
    arpu DECIMAL(10,3)  -- 平均每用户消费次数
);

INSERT INTO daily_arpu
SELECT 
    t1.date,
    t1.total_buy_count,
    t2.active_user_count,
    ROUND(t1.total_buy_count / t2.active_user_count, 3) AS arpu
FROM 
    -- 计算每日总购买次数
    (SELECT 
        date,
...
运行结果如下：
![ARPU计算](customer_beauty_images/ARPU.png)

#### (2)用户ARPPU指标计算：

以此类推，ARPPU定义为：每日总购买次数除以当日活跃付费用户数（活跃付费用户指当日有购买行为的用户）。可以看出和ARPU唯一的区别是分母，ARPU的分母是活跃用户数（包含4种行为类型），ARPPU的分母是活跃付费用户数。

```sql
-- 创建每日ARPPU指标表
CREATE TABLE daily_arppu (
    date CHAR(10),
    total_buy_count INT,  -- 当日总购买次数
    paying_user_count INT,  -- 当日活跃付费用户数
    arppu DECIMAL(10,3)  -- 平均每付费用户消费次数
);

INSERT INTO daily_arppu
SELECT 
    t1.date,
    t1.total_buy_count,
    t2.paying_user_count,
    ROUND(t1.total_buy_count / t2.paying_user_count, 3) AS arppu
FROM 
    -- 复用每日总购买次数（与ARPU计算一致）
    (SELECT 
        date,
...
![ARPPU计算](customer_beauty_images/ARPPU.png)
...
- **原因分析**：商品品类转化表现分层明显，不同品类受用户偏好和认知影响，转化特征差异是比较显著的。
- **可落地建议**：高转化品类增加展示位与库存；高潜力品类通过“博主种草+卖点优化”提升曝光；低转化品类优化详情页或与热销品捆绑；低效品类评估淘汰，资源向高价值品类倾斜。

### 8、用户质量分析
![ARPU/ARPPU模型可视化](customer_beauty_images/美妆用户购物行为可视化-图片-11.jpg)
- **图表现象描述**：ARPU（粉色折线），在 0.055-0.076 区间波动，日常均值约 0.065-0.070。同样在 12 月 12 日出现异常峰值 0.27，是日常水平的 3-4 倍，其余时段趋势平缓。ARPPU（蓝色折线），整体在 1.15-1.29 区间波动，呈现 “高频波动但趋势平稳” 的特征。其中，12 月 12 日达到峰值 1.38，虽显著高于其他日期（如日常均值约 1.20），但没有拉开非常大的差距，作为双十一大促的成绩是不能满意的。
- **原因分析**：ARPPU曲线暴露了和购买频次分析那里所展示的同样的问题，那就是人均付费次数过低。还是那句话，这可能和用户对产品未形成强偏好或使用习惯，复购意愿低有关；同时数据集周期仅一个月，小于美妆产品的使用周期，统计时间窗口不足。当然我们本次ARPPU分析采用的口径并不是付费金额而是付费次数，也有可能用户在双十二当天对于高价格产品的偏好远大于其他日期也说不准，这样一来分析结果就正常了。
- **建议**：优化一下数据追踪，延长统计周期。